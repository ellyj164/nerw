...
class Stripe extends BasePayment
{
	...
	protected function prepareDataForOneTimePayment($data): array
	{
		...
		if ($this->config->get('save_payment_method')) {
			...
			$returnData['consent_collection'] 						 = ['payment_method_reuse_agreement' => ['position' => 'auto']];
	...
	public function createWebhook()
	{
		try {
			...
			$exists = Arr::make($hooks->data)->some(function($value) use($webhookUrl) {
				return $value->url === $webhookUrl->__toString();
			});
	 ...
	 * @throws Throwable
	 */
	protected function createNewWebhook(): object
	{
		$webhookUrl = Uri::getInstance($this->config->get('webhook_url'));
		$webhookUrl->setVar('payment_method', $this->config->get('name'));

		try {
			$newWebhook = $this->client->webhookEndpoints->create([
			...
			return (object) [
				...
				'webhook_secret' 	=> $newWebhook->secret
			];
		} catch (Throwable $error) {
			throw $error;
		}
	...
	{
		$previousPayload = json_decode($data->previous_payload)->data->object ?? json_decode($data->previous_payload);
	...
	protected function prepareDataForRefund($data) : array
	{
		...
		if ($paymentPayload->object !== 'payment_intent' || !isset($paymentPayload->id)) {
		...
		return [
			...
			'payment_intent' => $paymentPayload->id,
		];
	}
	...
	private function processPayment($event, &$returnData) : void
	{
		...
		$extraCharges 	= static::calculateExtraCharges($data, $currency);
		$paymentIntent 	= $data->object === 'charge' ? $this->client->paymentIntents->retrieve($data->payment_intent) : null;

		$returnData->id 				= $data->metadata->order_id;
		$returnData->payment_status 	= $event->status;
		$returnData->transaction_id 	= !is_null($paymentIntent) ? $paymentIntent->id : $data->id;
		$returnData->payment_method 	= $this->config->get('name');
		$returnData->tax_amount 		= !is_null($paymentIntent) ? static::getTaxAmount($paymentIntent) : static::getTaxAmount($data);
		...
		$returnData->payment_payload 	= json_encode($paymentIntent);
	...
	{
		$checkoutSessionData 	= $this->client->checkout->sessions->all(['payment_intent' => $data->id]);
	 ...
	 * @since   1.0.0
	 */
	private function calculateExtraCharges($chargeDetails, $currency): array
	{
		...
		if (!empty($chargeDetails->balance_transaction)) {
			...
			if (strtoupper($currency )!== strtoupper($balanceTransaction->currency)) {
				$processingFee 	= Helper::convertAmountByCurrency($processingFee, $currency, $balanceTransaction);
				$earnings 		= Helper::convertAmountByCurrency($earnings, $currency, $balanceTransaction);
			} 
		}
